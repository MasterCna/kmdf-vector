...
void*
reallocEx(void* ptr, size_t size)
{
    void* new;

    if (!ptr) {
        new = ExAllocatePool(POOL_COLD_ALLOCATION, size);
        if (!new) { goto error; }
    }
    else {
        if (MemSize(ptr) < size) {
...
bool vc_vector_realloc(vc_vector* vector, size_t new_count) {
    ...
    char* new_data = (char*)reallocEx(vector->data, new_size);
    if (!new_data) {
        return false;
    }
...
{
    vc_vector* v = (vc_vector*)ExAllocatePoolWithTag(POOL_COLD_ALLOCATION, sizeof(vc_vector), 0x22);
    if (v != NULL) {
        KdPrint(("Inside Vector.c\n"));
        v->data = NULL;
        v->count = 0;
